...
- A save reset is acceptable to simplify the change (if needed).
...
- Migration strategy:
  - Option A (preferred): clean save reset to avoid complex transitional migration.
  - Option B (fallback): best-effort migration (legacy `Combat` -> `Roaming`) if reset is not applied.
- Dungeon Combat XP grant:
  - Gain at floor clear + boss kill bonus.
...
- Dungeon runtime:
  - Add Combat XP grants during run (floor clear + boss kill).
...
- Save:
  - Reset path validated (or migration path validated if chosen).
...
- Final reset vs migration choice based on product constraints (player communication, implementation cost).
