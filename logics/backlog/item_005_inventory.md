...
- Gold becomes global to the account (not per player).

# Defaults (proposal)
- Panel is open by default (like Action status); toggle via the button under "recruit new hero".
- Item IDs: `meat`, `food`.
- Rates: Hunting +1 meat per completed action; Cooking -1 meat and +1 food per completed action.
- Blocking: Cooking is disabled when meat is missing; if running and meat hits 0, auto-stop.
- Gold is shown once as a global stat, removed from per-player display.
